...
@Component({
  selector: 'app-qc-rep-mirtest-results',
  templateUrl: './qc-rep-mirtest-results.component.html',
  styleUrls: ['./qc-rep-mirtest-results.component.scss']
})
export class QcRepMIRTestResultsComponent implements OnInit {
  public details:any;
  ...
  constructor(private qc:QcRepMaterialsService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.qc.getQCMaterialResults(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
public listdat:Date=new Date();
  ...
  constructor(private acc:AccAccountsService,private adm:AdminGeneralInfoService,private ino:CrmOrdersService,private par:PartyDetailsService,
    private modalService: NgbModal, private  admusrservice:AccAccountsService,
  ...
  this.ino.setSaleOrder(header,lines,term, taxe,3).subscribe(res =>
      {
        ...
        if(result.result=='OK')
        {
          ...
          this.listAdd();
          this.makeCle();
          this.close();//added by amrutha for redirect CRM - Pre sales - Enquiries ( Customer details aren't appearing after selecting the edit option)
          this.opened=false;
...
public ShowOrders()
{
  ...
  var dat2=dat1;
  this.ino.getSaleOrders(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(res =>
    {
...
public findAddresses(e)
{
  ...
  this.loadquotations(det[0].partyName);
  if(det)
  {
    this.supdetail={
...
loadquotations(customername:any){
  this.par.Getcrmquotationsbycustomers(customername).subscribe(res =>
    {

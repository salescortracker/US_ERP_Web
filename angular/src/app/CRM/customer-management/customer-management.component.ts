...
//     this.admusrservice.updateCRMCustomer(this.leadcreateform).subscribe(res=>{
//       this.close();
//      this.adm.showMessage("Customer  details updated successfully","success",1)
//     })
//   }
//   else{
//   this.admusrservice.SaveCRMCustomer(this.leadcreateform).subscribe(res=>{
//     this.close();
//    this.adm.showMessage("Customer  details saved successfully","success",1)
//   })
// }
// }
// }
...
savecrmlead() {
  ...
  if (this.checkValueForLead()) {
    ...
    if (this.leadcreateform.id > 0) {
      this.admusrservice.updateCRMCustomer(this.leadcreateform).subscribe(
        (res) => {
          console.log(res);
          this.close();
          this.adm.showMessage("Customer  details updated successfully","success",1);
        ...
        (err) => {
          ...
          if (err.status === 400) {
            if (err.error === 'Company name already exists. Please enter a unique company name.') {
              ...
              this.adm.showMessage(err.error || 'Failed to update customer details', 'Error', 2);
            }
          } else {
            this.adm.showMessage('Failed to update customer details', 'Error', 2);
          }
        }
      );
    } else {
      debugger;
      this.admusrservice.SaveCRMCustomer(this.leadcreateform).subscribe(
        (res) => {
          console.log(res);
          this.close();
          this.adm.showMessage("Customer  details saved successfully","success",1);
        ...
        (err) => {
          ...
          if (err.status === 400) {
            if (err.error === 'Company name already exists. Please enter a unique company name.') {
              ...
              this.adm.showMessage(err.error || 'Failed to save customer details', 'Error', 2);
            }
          } else {
            this.adm.showMessage('Failed to save customer details', 'Error', 2);
          }
        }
      );
    }
...
itemList:any;
  ...
  close(){
    this.router.navigate(["/crm/customerListing"]);
  }
  getLeadsbyid(): void {
    ...
    debugger;
    this.acc.GetCRMAllCustomersById(this.leadbyidform).subscribe(
      (res) => {

...
@Component({
  selector: 'app-cus-quotation',
  templateUrl: './cus-quotation.component.html',
  styleUrls: ['./cus-quotation.component.scss']
})
export class CusQuotationComponent implements OnInit {
@Input() childData:any
  ...
  constructor(
    ...
    this.quotationDetails.customer_id = Number(this.leadId)
  ...
  ngOnInit(): void {
    if(this.childData>0){
      ...
      this.quotationDetails.customer_id=this.leadId;
    }
    this.getcontacts()
    this.getInfo()
  ...
  editQuotation(e, view) {
    this.enquiry = view
    this.traCheck = 2
    this.admusrservice.GetCRMRxQuotation(e).subscribe(
      (res) => {
        console.log(res);
        this.myList = res.quotationLines
        this.quotationDetails = res.quotation
        this.leadDetails = res.leadDetails
        this.quotationDetails.quotationDate = new Date(res.quotation.quotationDate).getFullYear()+'-'+new Date(res.quotation.quotationDate).getMonth()+1+'-'+(new Date(res.quotation.quotationDate).getDate());//new Date(this.quotationDetails.quotationDate).toISOString().split('T')[0];
        this.quotationDetails.quotationValidity = new Date(res.quotation.quotationValidity).getFullYear()+'-'+new Date(res.quotation.quotationValidity).getMonth()+1+'-'+(new Date(res.quotation.quotationValidity).getDate());//new Date(this.quotationDetails.quotationValidity).toISOString().split('T')[0];
        for (let i = 0; i < this.myList.length; i++) {
          this.myList[i].units = this.totalunits.filter(a => a.recordId == this.myList[i].itemId);
        }
      },
  ...
  getInfo() {
    this.admusrservice.GetCRMRxQuotations(this.leadId).subscribe(
      (res) => {
  ...
  submitEnquiry() {
    if (this.valCheck()) {
      ...
      console.log(req);
      this.admusrservice.AddCRMRxQuotation(req).subscribe(
        (res) => {
          console.log(res);
          this.getInfo()
    ...
    if (e == 1) {
      this.crmLead.CreateSoFromQuotation(this.quotationDetails.quotationId).subscribe(
        (res) => {
          ...
          this.router.navigate(['/crm/crmcusmain/' + this.leadId], { queryParams: { tab: '7', soId: res.soId } });
        },
        (error) => {
          console.error('Error fetching contacts:', error);
        }
      ...
      this.router.navigate(['/crm/crmcusmain/' + this.leadId], { queryParams: { tab: '7', quotationid: this.quotationDetails.quotationId } });
    }
  }
  ...
  calculEnquiryTotal() {
    ...
    this.quotationDetails.grandTotal = subtotal + Number(this.quotationDetails.additionalCharges)
  }
  contactChanged(e) {
  ...
  valCheck() {
    // console.log(this.quotationDetails,this.myList);

...
@Component({
  selector: 'app-leadview-orders',
  templateUrl: './leadview-orders.component.html',
  styleUrls: ['./leadview-orders.component.scss']
})
export class LeadviewOrdersComponent implements OnInit {
  @Input() childData:any
  ...
  constructor(
    ...
    this.soDetails.leadId = Number(this.leadId)
  ...
  editso(e, view) {
    this.enquiry = view
    this.traCheck = 2
    this.crmLead.GetCRMRxso(e).subscribe(
      (res) => {
        console.log(res);
  ...
  getInfo() {
    this.crmLead.GetCRMRxSos(this.leadId).subscribe(
      (res) => {
  ...
  submitEnquiry() {
    if (this.valCheck()) {
      ...
      console.log(req);
      this.crmLead.AddCRMRxSo(req).subscribe(
        (res) => {
          console.log(res);
          this.getInfo()
  ...
  valCheck() {
    ...
    if (!this.soDetails.paymentTerms) {
      this.adm.showMessage('So Payment Terms is Required', 'Warning', 3);
      return;
    }

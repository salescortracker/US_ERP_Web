...
@Component({
  selector: 'app-leadview-enquiry',
  templateUrl: './leadview-enquiry.component.html',
  styleUrls: ['./leadview-enquiry.component.scss']
})
export class LeadviewEnquiryComponent implements OnInit {
  ...
  constructor(
    ...
    this.enquiryDetails.leadId = Number(this.leadId)
  ...
  ngOnInit(): void {
    ...
    console.log("leadid"+this.childData);
  ...
  editEnquiry(e, view) {
    ...
    this.crmLead.GetCRMRxEnquiry(e).subscribe(
      (res) => {
        ...
        this.enquiryDetails.enquiryDate = new Date(res.enquiry.enquiryDate).getFullYear()+'-'+new Date(res.enquiry.enquiryDate).getMonth()+1+'-'+(new Date(res.enquiry.enquiryDate).getDate());
        this.enquiryDetails.followupDate = new Date(res.enquiry.followupDate).getFullYear()+'-'+new Date(res.enquiry.followupDate).getMonth()+1+'-'+(new Date(res.enquiry.followupDate).getDate());
  ...
  getInfo() {
    this.crmLead.GetCRMRxEnquiries(this.leadId).subscribe(
      (res) => {
  ...
  submitEnquiry() {
    if (this.valCheck()) {
      ...
      console.log(req);
      this.crmLead.AddCRMRxEnquiry(req).subscribe(
        (res) => {
          console.log(res);
          this.getInfo()
  ...
  valCheck() {
    ...
    if (this.enquiryDetails.enquiryDate > this.enquiryDetails.followupDate) {
      this.adm.showMessage('Follow-up Date should be greater than Enquiry date', 'Warning', 3);
      return;
    }
  ...
  createQuote(track: any) {
    if (track == 1) {
      this.crmLead.CreateQuotationFromEnquiry(this.enquiryDetails.enquiryId).subscribe(
        (res) => {
          console.log(res);
          this.adm.showMessage('Quotation Added successfully!', "success", 1);
          this.router.navigate(['/crm/LeadsView/' + this.leadId], { queryParams: { tab: '6', quotationId: res.quotationId } });
        },
        (error) => {
          console.error('Error fetching contacts:', error);
        });
      ...
      this.router.navigate(['/crm/LeadsView/' + this.leadId], { queryParams: { tab: '6', enquiryid: this.enquiryDetails.enquiryId } });
    }

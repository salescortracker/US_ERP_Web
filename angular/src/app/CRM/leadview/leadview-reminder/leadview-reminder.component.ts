...
@Component({
  selector: 'app-leadview-reminder',
  templateUrl: './leadview-reminder.component.html',
  styleUrls: ['./leadview-reminder.component.scss']
})
export class LeadviewReminderComponent implements OnInit {
@Input() childData:any
  leadId: any  
  newdisableReminder=false;
  newReminder: any = {
  ...
  close(){
      ...
      this.dis.getReminderById(recordId).subscribe(
        (res: any) => {
          ...
          this.newReminder = {
            ...
            reminderDate: res.reminderDate.split("T")[0],
            reminderTime: res.reminderTime.hours+':'+res.reminderTime.minutes,
          ...
          //this.newReminder.reminderDate=(res.reminderDate).toISOString().substring(0,10);
        },
        (error) => {
    ...
    saveRemindercheck(){
      ...
      if(!this.newReminder.reminder_type){
        this.admg.showMessage('Reminder Type is Required','Warning',3);
        return;
      }

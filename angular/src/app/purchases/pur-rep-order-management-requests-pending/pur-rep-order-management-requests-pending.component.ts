...
@Component({
  selector: 'app-pur-rep-order-management-requests-pending',
  templateUrl: './pur-rep-order-management-requests-pending.component.html',
  styleUrls: ['./pur-rep-order-management-requests-pending.component.scss']
})
export class PurRepOrderManagementRequestsPendingComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurPendingRequests(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

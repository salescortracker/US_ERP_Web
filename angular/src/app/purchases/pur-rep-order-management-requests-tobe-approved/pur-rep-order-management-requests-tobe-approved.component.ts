...
@Component({
  selector: 'app-pur-rep-order-management-requests-tobe-approved',
  templateUrl: './pur-rep-order-management-requests-tobe-approved.component.html',
  styleUrls: ['./pur-rep-order-management-requests-tobe-approved.component.scss']
})
export class PurRepOrderManagementRequestsTobeApprovedComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurRequestsTobeApproved().subscribe(
       async res => {
         var det:any=res;
           if(det)
         {

...
@Component({
  selector: 'app-pur-rep-order-management-enquiries-list',
  templateUrl: './pur-rep-order-management-enquiries-list.component.html',
  styleUrls: ['./pur-rep-order-management-enquiries-list.component.scss']
})
export class PurRepOrderManagementEnquiriesListComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getEnquiriesList(1,this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

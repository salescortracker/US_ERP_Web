...
@Component({
  selector: 'app-pur-rep-order-management-enquiries-expired',
  templateUrl: './pur-rep-order-management-enquiries-expired.component.html',
  styleUrls: ['./pur-rep-order-management-enquiries-expired.component.scss']
})
export class PurRepOrderManagementEnquiriesExpiredComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getEnquiriesList(3,this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

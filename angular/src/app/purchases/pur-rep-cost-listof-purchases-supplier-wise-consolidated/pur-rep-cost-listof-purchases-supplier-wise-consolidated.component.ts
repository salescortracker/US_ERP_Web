...
@Component({
  selector: 'app-pur-rep-cost-listof-purchases-supplier-wise-consolidated',
  templateUrl: './pur-rep-cost-listof-purchases-supplier-wise-consolidated.component.html',
  styleUrls: ['./pur-rep-cost-listof-purchases-supplier-wise-consolidated.component.scss']
})
export class PurRepCostListofPurchasesSupplierWiseConsolidatedComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.purRepPurchaseReturnSupplierWiseConsolidated(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
@Component({
  selector: 'app-pur-rep-stock-top-purchases',
  templateUrl: './pur-rep-stock-top-purchases.component.html',
  styleUrls: ['./pur-rep-stock-top-purchases.component.scss']
})
export class PurRepStockTopPurchasesComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepStocksService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getTop10Purchases(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

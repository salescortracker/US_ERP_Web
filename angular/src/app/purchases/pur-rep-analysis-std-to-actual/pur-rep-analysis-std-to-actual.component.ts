...
@Component({
  selector: 'app-pur-rep-analysis-std-to-actual',
  templateUrl: './pur-rep-analysis-std-to-actual.component.html',
  styleUrls: ['./pur-rep-analysis-std-to-actual.component.scss']
})
export class PurRepAnalysisStdToActualComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepAnalysisService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getAnalysisPriceComparisonWithStdPrice(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

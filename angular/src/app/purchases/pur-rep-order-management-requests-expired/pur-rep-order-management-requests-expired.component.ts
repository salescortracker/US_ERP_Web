...
@Component({
  selector: 'app-pur-rep-order-management-requests-expired',
  templateUrl: './pur-rep-order-management-requests-expired.component.html',
  styleUrls: ['./pur-rep-order-management-requests-expired.component.scss']
})
export class PurRepOrderManagementRequestsExpiredComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurRequestsExpired().subscribe(
       async res => {
         var det:any=res;
           if(det)
         {

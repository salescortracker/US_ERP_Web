...
@Component({
  selector: 'app-pur-rep-order-management-pending-advances',
  templateUrl: './pur-rep-order-management-pending-advances.component.html',
  styleUrls: ['./pur-rep-order-management-pending-advances.component.scss']
})
export class PurRepOrderManagementPendingAdvancesComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurPendingAdvances().subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

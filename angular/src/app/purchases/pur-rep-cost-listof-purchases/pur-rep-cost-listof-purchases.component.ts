...
@Component({
  selector: 'app-pur-rep-cost-listof-purchases',
  templateUrl: './pur-rep-cost-listof-purchases.component.html',
  styleUrls: ['./pur-rep-cost-listof-purchases.component.scss']
})
export class PurRepCostListofPurchasesComponent implements OnInit {
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getListOfPurchases(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

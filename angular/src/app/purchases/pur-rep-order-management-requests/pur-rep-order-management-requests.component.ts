...
@Component({
  selector: 'app-pur-rep-order-management-requests',
  templateUrl: './pur-rep-order-management-requests.component.html',
  styleUrls: ['./pur-rep-order-management-requests.component.scss']
})
export class PurRepOrderManagementRequestsComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurRequests(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
const now = new Date();
@Component({
  selector: 'app-pur-rep-cost-listof-purchases-consolidated',
  templateUrl: './pur-rep-cost-listof-purchases-consolidated.component.html',
  styleUrls: ['./pur-rep-cost-listof-purchases-consolidated.component.scss']
})
export class PurRepCostListofPurchasesConsolidatedComponent implements OnInit {
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getListOfPurchasesConsolidated(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

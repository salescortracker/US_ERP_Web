...
@Component({
  selector: 'app-pur-rep-stock-least-purchases',
  templateUrl: './pur-rep-stock-least-purchases.component.html',
  styleUrls: ['./pur-rep-stock-least-purchases.component.scss']
})
export class PurRepStockLeastPurchasesComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepStocksService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getLeast10Purchases(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

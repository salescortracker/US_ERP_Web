...
@Component({
  selector: 'app-pur-rep-order-management-supplier-wise-advances',
  templateUrl: './pur-rep-order-management-supplier-wise-advances.component.html',
  styleUrls: ['./pur-rep-order-management-supplier-wise-advances.component.scss']
})
export class PurRepOrderManagementSupplierWiseAdvancesComponent implements OnInit {
  ...
  constructor(private pur:PurRepOrderManagementService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getPurPendingAdvancesSupplierwise().subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

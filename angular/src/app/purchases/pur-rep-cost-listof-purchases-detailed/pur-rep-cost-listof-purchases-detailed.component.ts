...
@Component({
  selector: 'app-pur-rep-cost-listof-purchases-detailed',
  templateUrl: './pur-rep-cost-listof-purchases-detailed.component.html',
  styleUrls: ['./pur-rep-cost-listof-purchases-detailed.component.scss']
})
export class PurRepCostListofPurchasesDetailedComponent implements OnInit {
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getListOfPurchasesDetailed(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
@Component({
  selector: 'app-pur-rep-stock-no-purchases',
  templateUrl: './pur-rep-stock-no-purchases.component.html',
  styleUrls: ['./pur-rep-stock-no-purchases.component.scss']
})
export class PurRepStockNoPurchasesComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepStocksService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.getNoItemsPurchases(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

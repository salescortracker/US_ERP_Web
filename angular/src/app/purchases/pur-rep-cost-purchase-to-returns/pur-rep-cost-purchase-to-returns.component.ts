...
@Component({
  selector: 'app-pur-rep-cost-purchase-to-returns',
  templateUrl: './pur-rep-cost-purchase-to-returns.component.html',
  styleUrls: ['./pur-rep-cost-purchase-to-returns.component.scss']
})
export class PurRepCostPurchaseToReturnsComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.purRepPurchaseToPurchaseReturn(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
@Component({
  selector: 'app-pur-rep-cost-consolidated-purchase-returns',
  templateUrl: './pur-rep-cost-consolidated-purchase-returns.component.html',
  styleUrls: ['./pur-rep-cost-consolidated-purchase-returns.component.scss']
})
export class PurRepCostConsolidatedPurchaseReturnsComponent implements OnInit {
  public details:any;
  ...
  constructor(private pur:PurRepPurchasesService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.pur.purRepPurchaseReturnsConsolidated(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

...
@Component({
  selector: 'app-acc-customer-attachments',
  templateUrl: './acc-customer-attachments.component.html',
  styleUrls: ['./acc-customer-attachments.component.scss']
})
export class AccCustomerAttachmentsComponent implements OnInit {
  ...
  public banks:any=[];
    ...
    constructor(private acc:AccAccountsService,private adm:AdminGeneralInfoService, private spinner: NgxSpinnerService) {
  

      if(this.adm.screenCheck(1,1,4,0))
      {
      this.acc.getTreeWiseAccounts('SUNDRY DEBITORS').subscribe(res =>{

...
import { SaleReportsService } from 'app/services/sales/sale-reports.service';
const now = new Date();

@Component({
  selector: 'app-sal-rep-consolidated-sales',
  templateUrl: './sal-rep-consolidated-sales.component.html',
  styleUrls: ['./sal-rep-consolidated-sales.component.scss']
})
export class SalRepConsolidatedSalesComponent implements OnInit {
  ...
  constructor(private sal:SaleReportsService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.sal.getListOfSalesConsolidated(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)

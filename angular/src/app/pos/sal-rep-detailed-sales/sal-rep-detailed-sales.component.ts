...
@Component({
  selector: 'app-sal-rep-detailed-sales',
  templateUrl: './sal-rep-detailed-sales.component.html',
  styleUrls: ['./sal-rep-detailed-sales.component.scss']
})
export class SalRepDetailedSalesComponent implements OnInit {
  ...
  constructor(private sal:SaleReportsService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.sal.getListOfSalesDetailed(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)
